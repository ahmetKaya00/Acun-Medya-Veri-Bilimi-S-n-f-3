...
def mesaj_goster(mesaj, sure=2):
    ...
    while time.time() - baslangic < sure:
        ret, img = cap.read()
        cv2.putText(img, mesaj, (50,200),cv2.FONT_HERSHEY_SIMPLEX,1.5,(255,255,0),3)
        cv2.imshow("Parmak Okuma", img)

        if cv2.waitKey(1) & 0xFF == 27:
            break

mesaj_goster("1. Parginizi Gosterin",2)
ilk_parmak = parmak_sayisi_al(3)
mesaj_goster("tamamdir",2)

mesaj_goster("2. Parginizi Gosterin",2)
...
while True:
    ...
    cv2.putText(img,f"Toplam: {toplam}",(50,200),cv2.FONT_HERSHEY_SIMPLEX,1,(255,255,0),2)
